...
def copiar(tablero):
    #devuelve una copia del parametro que recibe(un tablero)
    numeros  = []
    completo = []
    for i in tablero:
        for j in i:
...
def inferencia(tablero):
    ...
    if x != None and y != None and isinstance(posibles[x][y],list):
        ...
        # si el for anterior finaliza sin haber dado una respuesta
        # significa que recorrio las posibles ramas y ninguna cumplio 
